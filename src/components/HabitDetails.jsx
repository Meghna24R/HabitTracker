//component that displays the habit details that user have already added
...
const HabitDetails = ({ habit, habit: { details } }) => {
  ...
  //status is none by default and is changed to done when clicked once and fail when clicked twice
      ...
      {details.map((detail) => (
        <Fragment key={detail.day}>
          <Col >
            ...
            {/* to check and status and display accordingly */}
            {detail.status === 'none' && (
              <i
